    ...
    for i, source in enumerate(sources):
        ...
        for j, sentence in enumerate(source):
            ...
            if has_image and "<image>" in sentence["value"]:
                ...
                _input_id = (
                    tokenizer(role).input_ids + nl_tokens * 2 + tokenizer(sentence["value"]).input_ids + [eot_id]
                )
            input_id += _input_id
...
class DPODataset(Dataset):
    ...
    def process_image(self, image_file):
        ...
        return (image_tensor, image_size, "image"), pruning_info
        ...
        if "image" in sources[0]:
            ...
            if suffix == "pkl":
                ...
                input_prompt = input_prompt.replace(
                    DEFAULT_IMAGE_TOKEN, DEFAULT_IMAGE_TOKEN * self.data_args.video_token
                )
                    ...
                    if self.data_args.frames_upbound > 0:
                        if len(frame_idx) > self.data_args.frames_upbound:
                            uniform_sampled_frames = np.linspace(
                                0, total_frame_num - 1, self.data_args.frames_upbound, dtype=int
                            )
...
def get_model(model_args, training_args, bnb_model_from_pretrained_args):
    ...
    if model_args.model_class_name is not None:
        ...
        if "mixtral" in model_args.model_name_or_path.lower():
            ...
            or "longva" in model_args.model_name_or_path.lower()
        ):
        ...
        if data_args.image_grid_pinpoints is not None:
            ...
            if "x" in data_args.image_grid_pinpoints and "..." in data_args.image_grid_pinpoints:
                ...
                data_args.image_grid_pinpoints = [
                    [int(x) * vis_encoder_size for x in item.split(",")] for item in grid_pinpoints
                ]
            else:
