...
# -*- coding: utf-8 -*-
"""
python -u ps3-style-cut-model/longva/scripts/eval_mme_remote_sensing_multiscale.py \
  --ckpt checkpoint/LongVA-multiscale-selected \
  --data_json mme_remote_sensing_conversations.json \
  --image_root /path/to/MME-RealWorld-Lite/imgs \
  --mask_root /path/to/multiscale_tiles_masks_mme \
  --output_json checkpoint/LongVA-multiscale-selected/result/multiscale_results_mme.jsonl \
  --multiscale_topk 80,320,1600,4000 \
  --max_samples 0 \
  --devices cuda:2,cuda:3,cuda:1,cuda:5,cuda:6,cuda:4,cuda:7
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--data_json",
        required=True,
        help="Path to MME-RealWorld-Lite JSON.",
    )
    parser.add_argument(
        "--image_root",
        required=True,
    ...
    parser.add_argument(
        "--mask_root",
        required=True,
        help="Directory of multiscale tile masks (.pt) from build_multiscale_tile_masks.py.",
    )
...
def run_worker(
    ...
    for idx in indices:
        ...
        try:
            ...
            raw_gen = _strip_leading_image_tokens(raw_gen)
            clean_tail = _clean_generated_text(raw_gen)
            clean_full = _clean_generated_text(_strip_leading_image_tokens(raw_full))
            seq_resp = decode_chatml_response(tokenizer, sequences)
            gen_resp = decode_chatml_response(tokenizer, gen_tokens)
            pred_text = next(
                (
                    txt
                    for txt in (
                        clean_tail,
                        _clean_generated_text(_strip_leading_image_tokens(gen_resp)),
                        _clean_generated_text(_strip_leading_image_tokens(seq_resp)),
                        clean_full,
                    )
                    if txt
                ),
            ...
            pred_choice = _extract_single_choice_letter(pred_text, letters_used)
            pred_choices = _extract_choice_set(pred_text, letters_used)
            ...
            if pred_choices:
                ...
                clean_output = _first_nonempty_line(pred_text or clean_tail or clean_full)
            ...
            result = {
                ...
                "raw_output": pred_text or clean_tail or clean_full,
                "raw_tail": clean_tail or pred_text or clean_full,
                "image_sizes": image_sizes,
