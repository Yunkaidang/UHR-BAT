...
_ARCH_PATCH_MODULE = None


def _ensure_llava_arch_methods(llava_causal_lm_model):
    """
    Some environments may load an older longva package lacking multiscale helpers.
    Dynamically load the current llava_arch.py and bind missing methods onto base_model.
    """
    global _ARCH_PATCH_MODULE
    if _ARCH_PATCH_MODULE is None:
        arch_path = Path(__file__).resolve().parents[1] / "model" / "llava_arch.py"
        spec = importlib.util.spec_from_file_location("longva.model.llava_arch_patch", arch_path)
        module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(module)
        _ARCH_PATCH_MODULE = module

    provider_cls = None
    for candidate in ["LlavaMetaForCausalLM", "LlavaMetaModel"]:
        if hasattr(_ARCH_PATCH_MODULE, candidate):
            provider_cls = getattr(_ARCH_PATCH_MODULE, candidate)
            break
    ...
    for name in [
        "select_tokens_with_masks",
        "prepare_inputs_with_selected_tokens",
        "compute_multiscale_image_attention",
        "initialize_vision_tokenizer",
    ]:
        if hasattr(provider_cls, name):
            fn = getattr(provider_cls, name)
            if not hasattr(llava_causal_lm_model, name):
                setattr(llava_causal_lm_model, name, types.MethodType(fn, llava_causal_lm_model))
            if hasattr(llava_causal_lm_model, "get_model"):
                inner = llava_causal_lm_model.get_model()
                if not hasattr(inner, name):
                    setattr(inner, name, types.MethodType(fn, inner))
...
class LLaVATrainer(Trainer):
    def training_step(self, model, inputs):
        """
        Customized training step with defensive guards:
        - Skip steps when loss is NaN/Inf (zero grad).
        - Clip gradients (when not using DeepSpeed, which handles clipping internally).
        """
        model.train()
        ...
        with self.accelerator.accumulate(model):
            ...
            if torch.isnan(loss) or torch.isinf(loss):
                if getattr(self.args, "local_rank", -1) in (-1, 0):
                    rank0_print("[Warn] Loss is NaN/Inf; zeroing gradients and skipping this step.")
            ...
            if max_grad_norm is not None and max_grad_norm > 0 and not self.deepspeed:
                grad_norm = self.accelerator.clip_grad_norm_(model.parameters(), max_grad_norm)
                if grad_norm is not None and (torch.isnan(grad_norm) or torch.isinf(grad_norm)):
                    if getattr(self.args, "local_rank", -1) in (-1, 0):
                        rank0_print("[Warn] Grad norm NaN/Inf; zeroing gradients and skipping this step.")
                    if self.optimizer is not None:
                        self.optimizer.zero_grad(set_to_none=True)
                    else:
                        model.zero_grad(set_to_none=True)
    ...
    def create_accelerator_and_postprocess(self):
        ...
        process_group_kwargs = InitProcessGroupKwargs(timeout=timedelta(weeks=52))
        ...
        if dispatch_batches is not None:
            ...
            if "dispatch_batches" in accelerator_signature.parameters:
                accelerator_init_kwargs["dispatch_batches"] = dispatch_batches
            elif dispatch_batches:
                rank0_print(
                    "当前 accelerate 版本不支持 dispatch_batches 参数，已忽略该设置；如需此功能请升级 accelerate。"
                )
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        if getattr(self.args, "tune_mm_mlp_adapter", False) or (
            ...
            try:
                # 旧版本 transformers（带 metrics 形参）
                super(LLaVATrainer, self)._save_checkpoint(model, trial, metrics)
            except TypeError:
                try:
                    # 新版本 transformers（不带 metrics）
                    super(LLaVATrainer, self)._save_checkpoint(model, trial)
                except TypeError:
                    # 极端情况下再降级（极少见）
                    super(LLaVATrainer, self)._save_checkpoint(model)
