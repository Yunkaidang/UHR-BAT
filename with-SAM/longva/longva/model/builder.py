...
def load_pretrained_model(
    ...
    **kwargs,
    ...
    def _apply_attention_implementation(m):
        ...
        for attr in ("model", "language_model", "base_model"):
            ...
            if inner is not None and hasattr(inner, "config"):
                setattr(inner.config, "_attn_implementation", attn_implementation)
                if hasattr(inner.config, "attn_implementation"):
                    setattr(inner.config, "attn_implementation", attn_implementation)
            if inner is not None and hasattr(inner, "layers"):
                for blk in getattr(inner, "layers", []):
                    attn = getattr(blk, "self_attn", None)
                    if attn is not None:
                        attn.attn_implementation = attn_implementation
    ...
    _apply_attention_implementation(model)
    ...
    if "llava" in model_name.lower() or "longva" in model_name.lower() or is_multimodal:
        ...
        # Respect caller's device_map when moving the vision tower.
        # If a specific CUDA device is provided (e.g., "cuda:4"), use it instead of the default "cuda".
        if device_map != "auto":
            if isinstance(device_map, str):
                target_device = device_map
            else:
                # Fallback: keep previous behavior but avoid crashing on CPU-only
