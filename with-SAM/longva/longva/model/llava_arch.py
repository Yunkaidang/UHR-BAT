...
class LlavaMetaForCausalLM(ABC):
    ...
    def select_tokens_with_masks(
        ...
        """
        Select tokens per scale based on attention and segmentation masks.
        Background (mask==0) tokens are treated as individual classes.
        Foreground classes use average attention; within each class, tokens above the global mean are taken individually,
        remaining tokens are averaged into a single token.
        """
        if target_sizes is None:
        ...
        for scale_idx, scale in enumerate(target_sizes):
            ...
            raw_mask = masks_by_scale.get(int(scale))
            mask_map = None
            if isinstance(raw_mask, dict):
                if isinstance(raw_mask.get("masks"), torch.Tensor):
                    mask_map = raw_mask.get("masks")
                elif isinstance(raw_mask.get("mask"), torch.Tensor):
                    mask_map = raw_mask.get("mask")
            else:
                mask_map = raw_mask
            if mask_map is not None and not isinstance(mask_map, torch.Tensor):
                mask_map = torch.as_tensor(mask_map)
            token_embeds = token_embeds_by_scale.get(int(scale))
            if attn_map is None or token_embeds is None:
                continue
            # attn_map: (tiles, ts, ts); mask_map: (tiles, ts, ts) if provided; token_embeds: (tiles, tokens, hidden)
            ...
            if mask_map is not None:
                # Normalize potential flattened caches to (tiles, ts, ts)
                mask_map = mask_map.to(attn_map.device)
                total_tokens = tokens_per_tile
                if mask_map.dim() == 1 and mask_map.numel() == tiles * total_tokens:
                    mask_map = mask_map.view(tiles, ts, ts)
                elif mask_map.dim() == 2 and mask_map.shape[1] == total_tokens:
                    mask_map = mask_map.view(mask_map.shape[0], ts, ts)
                elif mask_map.dim() == 3 and mask_map.shape[1] != ts and mask_map.shape[2] != ts:
                    # Avoid silently reshaping inconsistent 3D masks
                    raise RuntimeError(
                        f"[Error] 掩码尺寸异常: scale={scale}, attn_map={tuple(attn_map.shape)}, mask_map={tuple(mask_map.shape)}"
                    )
                if mask_map.dim() != 3:
                    raise RuntimeError(
                        f"[Error] 掩码尺寸无效: scale={scale}, attn_map={tuple(attn_map.shape)}, mask_map={tuple(mask_map.shape)}"
                    )
                if mask_map.shape[0] > tiles:
                    mask_map = mask_map[:tiles]
                if mask_map.shape != (tiles, ts, ts):
                    raise RuntimeError(
                        f"[Error] 掩码与注意力尺寸不匹配: scale={scale}, attn_map={tuple(attn_map.shape)}, mask_map={tuple(mask_map.shape)}"
                    )
            ...
            if getattr(self.config, "_log_vision_tokens", False):
                try:
                    mask_nz = int(mask_map.sum().item()) if mask_map is not None else None
                    rank0_print(
                        f"[Debug] Scale {scale}: attn_shape={tuple(attn_map.shape)}, mask_nz={mask_nz}, "
                        f"tokens_per_tile={tokens_per_tile}, tiles={tiles}, k_limit={k_limit}"
                    )
                except Exception:
                    pass
            ...
            flat_mask = mask_map.reshape(-1) if mask_map is not None else None
            ...
            if flat_mask is not None and flat_mask.numel() != flat_attn.numel():
                raise ValueError(
                    f"[Error] scale {scale}: mask tokens {flat_mask.numel()} != attn tokens {flat_attn.numel()}"
                )

            # 无 mask 时，纯按注意力取 top-k
            if mask_map is None:
                if k_limit <= 0:
                    continue
                k_actual = min(k_limit, embeds.shape[0])
                topk_scores, topk_idx = torch.topk(flat_attn, k_actual)
                selected_embeds = embeds[topk_idx]
                selected_meta = [
                    {"scale": int(scale), "pos": [int(idx)], "score": float(score)}
                    for idx, score in zip(topk_idx.tolist(), topk_scores.tolist())
                ]
                per_batch_embeds[0].append(selected_embeds)
                per_batch_indices[0].append({"scale": int(scale), "tokens": selected_meta})
                continue

            bg_mask = flat_mask == 0
            bg_positions = torch.nonzero(bg_mask, as_tuple=False).squeeze(-1)
            bg_scores = flat_attn[bg_mask]

            candidates: List[tuple] = []
            for pos, score in zip(bg_positions.tolist(), bg_scores.tolist()):
                candidates.append(("bg", float(score), pos))

            fg_ids = torch.unique(flat_mask[flat_mask > 0]).tolist()
            for cid in fg_ids:
                cid_tensor = flat_mask == cid
                pos_idx = torch.nonzero(cid_tensor, as_tuple=False).squeeze(-1)
                if pos_idx.numel() == 0:
                    continue
                score = float(flat_attn[pos_idx].mean())
                candidates.append(("fg", score, pos_idx))
            ...
            for ctype, score, payload in candidates:
                if remaining <= 0:
                    break
                if ctype == "bg":
                    pos_idx = int(payload)
                    selected_embeds.append(embeds[pos_idx].unsqueeze(0))
                    selected_meta.append({"scale": int(scale), "pos": [pos_idx], "score": score})
                    remaining -= 1
                    continue

                pos_idx: torch.Tensor = payload
                class_attn = flat_attn[pos_idx]
                class_mean = float(class_attn.mean())
                # Keep tokens with attention above their own class mean; aggregate the rest.
                ...
                if low_mask.any() and remaining > 0:
                    ...
                    selected_meta.append(
                        {"scale": int(scale), "pos": low_pos.tolist(), "score": mean_score, "aggregated": True}
                    )
                    remaining -= 1
    ...
    def compute_multiscale_image_attention(
        ...
        with torch.no_grad():
            ...
            rotary_forward = getattr(rotary, "forward", None)
            use_seq_len_kw = True
            if rotary_forward is not None:
                try:
                    sig = inspect.signature(rotary_forward)
                    use_seq_len_kw = "seq_len" in sig.parameters
                except (ValueError, TypeError):
                    use_seq_len_kw = True
            if use_seq_len_kw:
                position_embeddings = rotary(hidden_before_last, seq_len=hidden_before_last.shape[1])
            else:
                position_embeddings = rotary(hidden_before_last, pos_ids)
            causal_mask = None
            try:
